...
A worker has two handles: a `tmux_name` (you choose at launch — e.g.
`task-$$`) and a `session_id` (a UUID generated by `launch-worker.sh`). Every
script that takes a worker handle accepts **either form** — internally it
looks the other up from `/tmp/claude-workers/<session-id>.meta`. Use whichever
is easier to remember in context; the docs lead with `session_id` because
that's what `launch-worker.sh` returns.
...
If you lose track of running workers, `scripts/list-workers.sh` joins
`tmux ls` with the meta files and shows alive/dead status. `scripts/current.sh`
prints the session_id of the most recently launched worker, for one-worker
flows where you don't want to capture and re-thread the UUID.
...
| `list-workers.sh` | (no args) | Show every known worker with alive/dead status |
